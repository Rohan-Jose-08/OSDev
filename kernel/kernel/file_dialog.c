...
static void fd_refresh_list(file_dialog_t* dialog) {
    ...
    // List files
    fs_dirent_t entries[FD_MAX_FILES];
    int count = fs_list_dir(dialog->current_path, entries, FD_MAX_FILES);
    
    for (int i = 0; i < count && dialog->file_count < FD_MAX_FILES; i++) {
        strncpy(dialog->files[dialog->file_count].name, entries[i].name, 31);
        dialog->files[dialog->file_count].name[31] = '\0';
        
        // Check if directory
        char full_path[128];
        snprintf(full_path, sizeof(full_path), "%s/%s", dialog->current_path, entries[i].name);
        fs_inode_t inode;
        if (fs_stat(full_path, &inode)) {
            dialog->files[dialog->file_count].is_directory = (inode.type == 2);
        } else {
            dialog->files[dialog->file_count].is_directory = false;
        }
        dialog->file_count++;
    }
}
...
static void fd_draw_ui(file_dialog_t* dialog) {
    ...
    // File list area
    int list_y = y;
    int list_width = win->content_width - 30; // Leave room for scroll bar
    ...
    window_fill_rect(win, 10, y, win->content_width - 20, FD_INPUT_HEIGHT, COLOR_WHITE);
    window_draw_rect(win, 10, y, win->content_width - 20, FD_INPUT_HEIGHT, 
    ...
    int ok_x = win->content_width / 2 - FD_BUTTON_WIDTH - button_spacing / 2;
    int cancel_x = win->content_width / 2 + button_spacing / 2;
...
static void fd_confirm(file_dialog_t* dialog) {
    ...
    // Clear user_data pointer in window so window_destroy doesn't free it
    dialog->window->user_data = NULL;
...
// Click handler
static void fd_on_click(window_t* win, int x, int y) {
    file_dialog_t* dialog = (file_dialog_t*)win->user_data;
    if (!dialog) return;
    
    // Calculate positions matching fd_draw_ui
    int list_y = 50;  // 10 (start) + 20 (title) + 20 (path)
    int list_width = win->content_width - 30;
    ...
    // Check button clicks
    int ok_x = win->content_width / 2 - FD_BUTTON_WIDTH - 5;
    int cancel_x = win->content_width / 2 + 5;
...
// Key handler
static void fd_on_key(window_t* win, char key) {
    file_dialog_t* dialog = (file_dialog_t*)win->user_data;
    if (!dialog) return;
...
void file_dialog_show_open(const char* title, 
    ...
    // Create dialog state
    file_dialog_t* dialog = (file_dialog_t*)kmalloc(sizeof(file_dialog_t));
    if (!dialog) {
        window_destroy(win);
        return;
    }
    ...
    win->on_click = fd_on_click;
    win->on_key = fd_on_key;
    win->on_scroll = fd_on_scroll;
